...
def get_all_forms(**kwargs):
    try:
        ...
        if forms:
            return {
                "message": "Forms found",
                "data": [form.serialize() for form in forms],
            }, 200
        else:
...
def get_complete_form(form_id, **kwargs):
    try:
        form = (
            ...
            .filter_by(id=form_id, user_id=kwargs["user"].id)
            .first()
        )
        if form:
    ...
    try:
        ...
        if form:
            if (
                ...
                .filter_by(form_id=form_id, user_id=kwargs["user"].id)
                .first()
            ):
            ...
            for field in fields:
                ...
                new_response_data = ResponseData(
            ...
            return {
                ...
                "data": get_form_response(form_id, kwargs["user"].id),
            }, 201
        return {"message": "Form not found"}, 404
    except Exception as e:
        db.session.rollback()
    ...
    try:
        ...
        if form:
            ...
            if gs.service:
                ...
                for resp in db.session.query(Response).filter_by(form_id=form_id).all():
                    ...
                    for field in fields:
                        response.append(
                            ...
                            .filter_by(response_id=resp.id, field_id=field.id)
                            .first()
                            .value
                        )
                ...
                return {
                    "message": "GoogleSheet Created",
                    "data": gsheet["spreadsheetUrl"],
                }, 200
