...
def get_form_response(form_id, user_id):
    try:
        response = db.session.query(Response).filter_by(form_id=form_id, user_id=user_id).first()
        if response:
            response_data = {}
            ...
            data=[]
            for field in fields:
                resp_data=db.session.query(ResponseData).filter_by(response_id=response.id,field_id=field.id).first()
...
def get_all_response(form_id):
    try:
        fields=db.session.query(Field).filter_by(form_id=form_id).all()
        data=[]
        responses=db.session.query(Response).filter_by(form_id=form_id).order_by(Response.created_at.desc()).all()
        for response in responses:
            ...
            resp_data['data']=[]
            for field in fields:
                resp=db.session.query(ResponseData).filter_by(response_id=response.id,field_id=field.id).first()
                resp_data['data'].append({'field':field.serialize(),'value':resp.value if resp else None })
